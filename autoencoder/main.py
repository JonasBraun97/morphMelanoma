...
parser.add_argument('--earlystop',		type=str,	default='train',			help='train or load model')
parser.add_argument('--inputDir',       type=str,   default='/Users/jbj7388/Library/CloudStorage/OneDrive-NorthwesternUniversity/ownProject/data/mergedDAPIYFPNormalized_Subregion_13_r2_c3/singleCellImages',     help='input data directory (in train subfolder)')
...
parser.add_argument('--weights',       type=str,   default='Outputs/checkpoints',     help='model weight directory')
...
def main():
    ...
    if args.mode == 'load':
        if args.checkpoint == 'NA':
            sys.exit('No weight file provided')
        model = CVAE(args)
        model.cvae.load_weights(args.weights)
        model.encode(aug_ds)
